...
echo "-    14. Dir Finder				-"
echo "-    15. WordPress Checker 			-"
echo "-    16. Mass Subdomain			-"
echo "-    0. Keluar    				-"
...
read -p "	Filename: " FN
for i in ${i[@]}; do
	response=$(curl -Lsk -l $url/files/journals/$i/articles/$nomer/submission/)
	if [[ $response =~ "Forbidden" ]] || [[ $response =~ "200 OK" ]] || [[ $response =~ "index of" ]] || [[ $response =~ "orbidden" ]] || [[ $response =~ "submit" ]] || [[ $response =~ "Shell" ]] || [[ $response =~ "pload" ]]
	then
...
function scan {
gas=$(curl -Lsk $1$2 -l)
	if [[ $gas =~ "ajax_calls.php?action=save_img" ]]
	then
		echo -e "\e[42m [Found] $1$2\e[0m"
...
function scan {
	curl_timeout=10
	multithread_limit=50
 hasil=$(curl --connect-timeout ${curl_timeout} --max-time ${curl_timeout} -Lsk "https://sonar.omnisint.io/subdomains/$1" | sed --separate 's/,/\n/g' | sed 's|"||g' | uniq -u)
 if [[ $hasil =~ "$1" ]]
 then
 echo -e "\033[0m Scanning \033[0;36m$1\033[0m"
  printf '%s\n' "$hasil" >> ResultBot/Subdo.txt
 else
  echo -e "\e[31m Invalid Subdomain \e[0m"
  fi
 }
  read -p "Masukan DomainList: " file
export -f scan 
parallel -j 5 scan :::: "$file"
